...
async function createData(blog) {
...
async function deleteData(blogID, userID) {
    ...
    await drop(blogInfo?.banner); //Go banner tren cloud 
...
async function updateData(blogID, payload, base64File) {
...
async function getDataForHome(page) {
    ...
    for (let i = 0; i < res.length; ++i)
        res[i].content = res[i].content.slice(0, 300);
    return res;
}
