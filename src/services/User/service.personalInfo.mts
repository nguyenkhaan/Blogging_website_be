...
async function updateData(id: string, payload: any, reqFile: any = undefined) {
    ...
    if (payload.avatar) {
        ...
        if (avatar != defaultAvatar) await drop(avatar as string); //Go di link hinh anh hien tai, nhung chi go khi no khong phai la hinh anh default
        const res = await upload(reqFile, userID);
        const url = res.url;
        await Promise.all([  //Hai promise chay cung luc, song song de tang toc do 
            mongodbPrisma.user.update({
                where: { userID },
                data: { avatar: url },
            }),
